...
<p>このリポジトリは、awkコマンドを実行して環境変数のPATHに設定されているパス群をawkでそれぞれのパスに分割し表示します</p>
</div>
...
<div class="wp-block-group">
<p>上記のコマンドをインストール済みの方は、以下のコマンドを実行してリポジトリからダウンロード後、ディレクトリを移動し、awkコマンドを実行してパスを表示してみてください</p>
...
<div class="wp-block-group">
<p>$PATHはシェルが実行ファイルを見つける際に検索にかけるパス群が格納されている環境変数です<br>:(コロン)で区切られているのでawkコマンドの-F(field separator)オプションで:を選択することで各パスに分割され、NF(number of fileds)変数でフィールド分のパスを表示しています<br>単純にパス群を表示したい場合は以下のようにechoコマンドの引数に$PATHを指定するだけです</p>
</div>



<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>echo $PATH</code></pre>
